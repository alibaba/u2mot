...
def gate_cost_matrix_by_cls(cost_matrix, atracks, btracks, super_cls: np.ndarray):
    """
    Compute cost-mask based on class (for multi-class multi-object tracking, e.g., bdd100k)
    :type atracks: list[STrack]
    :type btracks: list[STrack]
    ...
    if (len(atracks)>0 and isinstance(atracks[0], np.ndarray)) or (len(btracks) > 0 and isinstance(btracks[0], np.ndarray)):
        ...
        aclasses = [track.cls for track in atracks]
        bclasses = [track.cls for track in btracks]
    
    aclasses, bclasses = super_cls[np.array(aclasses).astype(np.int32)], super_cls[ np.array(bclasses).astype(np.int32)]
    mask = aclasses.reshape(-1, 1) != bclasses.reshape(1, -1)
    cost_matrix[mask] = 1.  # same super-category becomes 0
