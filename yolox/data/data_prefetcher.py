...
class DataPrefetcher:
    ...
    def preload(self):
        try:
            self.next_input, self.next_target, self.next_info, _ = next(self.loader)
    ...
    def next(self):
        ...
        input = self.next_input     # img: [1, 3, 3, 800, 1440], e.g. [batchsize, imgnum. channel, H, W]
        target = self.next_target   # annots: [1, 3, 1000, 6], e.g, [batchsize, imgnum, 1000, class_id + tlwh + track_id]
        info = self.next_info       # info: [1, n, 5], e.g. [height, width, frame_id, video_id, file_name]
